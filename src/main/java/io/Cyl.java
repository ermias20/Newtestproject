...
public class Cyl implements AndroidTest {
    ...
    public ExecutionResult execute(AndroidTestHelper helper) throws FailureException {
       ...
       Tolerance compare = new Tolerance();
        ...
        boolean Wfrightcompare = compare.rn(tolernce.newsph(TolRightSphere, TolRightCyl),
                Double.valueOf(driver.testproject().getText(by)), Tolerencevalue);
        BooleanCheck.ReporterCheck(report, Wfrightcompare, "tolernce comparison of expected and actual right sphere ");

        // 5.compare Tap 'right cyl' value with expected value and check tolerance
        by = By.id("current_cylinder_right_value");
        boolean Cylrightcompare = compare.rn(tolernce.newcyl(TolRightCyl),
                Double.valueOf(driver.testproject().getText(by)), Tolerencevalue);
        BooleanCheck.ReporterCheck(report, Cylrightcompare,
                "tolernce comparison of expected and actual right cylinder ");

        // 7.compare Tap 'left sphere' value with expected value and check tolerance
        by = By.id("current_sphere_left_value");
        boolean Wfleftcompare = compare.rn(tolernce.newsph(TolLeftSphere, TolLeftCyl),
                Double.valueOf(driver.testproject().getText(by)), Tolerencevalue);
        BooleanCheck.ReporterCheck(report, Wfleftcompare, "tolernce comparison of expected and actual left sphere ");

        // 8. compare Tap 'left cyl' value with expected value and check tolerance
        by = By.id("current_cylinder_left_value");
        boolean Cyleftcompare = compare.rn(tolernce.newcyl(TolLeftCyl),
                Double.valueOf(driver.testproject().getText(by)), Tolerencevalue);
